...
//    }
  ...
  private String createPrompt(List<Wear> userWears, WeatherInfoDTO weatherInfo) {
    ...
    prompt.append(
        "- The output must follow this exact JSON structure, ensuring that the items are ordered as top, bottom, and outerwear (if included):\n");
    prompt.append(
        "- Please make 10 combinations and explanations. If the wardrobe data is insufficient, feel free to reuse items to create the combinations.\n");
    prompt.append("{\n");
    prompt.append("  \"outfit\": [\n");
    prompt.append(
  ...
  private List<GPTRecommendationResponseDTO> parseGPTResponses(String response,
      List<Wear> userWears, Account account) {
    ...
    try {
      ...
      List<GPTRecommendationResponseDTO> responseDTOs = objectMapper.readValue(
          response, objectMapper.getTypeFactory()
              .constructCollectionType(List.class, GPTRecommendationResponseDTO.class)
      );
      ...
      for (GPTRecommendationResponseDTO dto : responseDTOs) {
        ...
        combination.setStyle(Style.valueOf(String.valueOf(
            dto.getOutfit().get(0).getStyle()))); // Use the style from the first outfit item
        combination.setWeather(Weather.valueOf(String.valueOf(
            dto.getOutfit().get(0).getSeason()))); // Use the season from the first outfit item
        ...
        for (GPTRecommendationResponseDTO.OutfitItemDTO item : dto.getOutfit()) {
          ...
          Wear matchedWear = userWears.stream()
              ...
              .orElseThrow(() -> new IllegalArgumentException(
                  "Wear with ID " + item.getId() + " not found in user's wardrobe"));
